...
export function BrowseContent({ initialPosts, initialPagination }: BrowseContentProps) {
  ...
  const [hasMore, setHasMore] = useState(
    initialPagination ? initialPagination.page < initialPagination.totalPages : true
  );
  ...
  const skipFirstClientFetch = useRef(!!initialPosts);
  ...
  // Fetch posts — skip on first mount if server provided initial data
  useEffect(() => {
    if (skipFirstClientFetch.current) {
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <div className="bg-[#800000] py-8 pb-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-8">
          ...
          {/* Search bar */}
          <form onSubmit={handleSearch} className="flex bg-white rounded-full shadow-lg p-1 pl-5 gap-2 items-center max-w-2xl relative">

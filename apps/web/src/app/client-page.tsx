...
export function HomeClient({ initialPosts, initialHasMore }: HomeClientProps) {
  const { user } = useAuth();
  const router = useRouter();
  const [recentPosts, setRecentPosts] = useState<PostCard[]>(initialPosts ?? []);
  ...
  // On the very first mount, skip the reset and fetch if server data was provided.
  const isFirstMount = useRef(true);

  // Reset pagination when tab or side changes (skip on first mount to preserve server data)
  useEffect(() => {
  ...
  useEffect(() => {
    if (isFirstMount.current) {
      ...
      if (initialPosts !== undefined) return; // use server-provided initial data
    }
